...
async function getFit(resultUrl: string, name: string, type: string, falKey: string) {
  ...
  try {
    const prompt =
      "You are a fashion expert. Rate how well the garment " +
      name +
      " (" +
      type.replace("_", " ") +
      ") fits this person on a scale of 1 to 100. Reply ONLY with this exact JSON format: {\"score\":85,\"tip\":\"One sentence tip here\"}";
    ...
    if (p && typeof p.score === "number") {
      return {
        score: Math.min(100, Math.max(1, p.score)),
        tip: p.tip || def.tip,
      };
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    /* FormData request (image upload) */
    else if (contentType.includes("multipart/form-data")) {
      const form = await req.formData();

      const humanFile = form.get("human_image") as File;
      const clothFile = form.get("cloth_image") as File;

      if (humanFile) {
        human_image = await fileToBase64(humanFile);
      }

      if (clothFile) {
        cloth_image = await fileToBase64(clothFile);
      }
    ...
    if (!human_image || !cloth_image) {
      return NextResponse.json(
        { error: "human_image and cloth_image required" },
        { status: 400 }
      );
    }
    ...
    while (Date.now() < deadline) {
      ...
      if (poll.status === "COMPLETED") {
          ...
          return NextResponse.json(
            {
              ...
              raw: JSON.stringify(poll).slice(0, 300),
            },
            { status: 502 }
          );
        }
        ...
        if (!resultUrl) {
          return NextResponse.json(
            { error: "Cannot extract image URL" },
            { status: 502 }
          );
        }

        const fit = await getFit(
          resultUrl,
          garment_name,
          cloth_type,
          FAL_KEY
      ...
      if (poll.status === "FAILED") {
        return NextResponse.json(
          { error: "Job failed: " + (poll.error || "unknown") },
          { status: 502 }
        );
      }
    }
    ...
    return NextResponse.json(
      { error: "Timed out after 55s, please try again" },
      { status: 504 }
    );
  } catch (err: any) {
